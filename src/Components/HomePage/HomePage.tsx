...
export const HomePage: FC = () => {
  ...
  const [techArticles, setTechArticles] = React.useState<NewsAPI['items']>([]);
  ...
  React.useEffect(() => {
    Promise.all<NewsAPI>([
      ...
      fetch(`https://frontend.karpovcourses.net/api/v2/ru/news/${categoryIds['tech']}`).then((res) =>
        res.json()
      ),
      fetch('https://frontend.karpovcourses.net/api/v2/ru/trends').then((res) => res.json()),
    ]).then(([articles, techArticles, trendArticles]) => {
      setArticles(articles.items);
      setTechArticles(techArticles.items);
      setTrendArticles(trendArticles.items);

      setCategories(
        [articles.categories, techArticles.categories, trendArticles.categories]
          .flat()
      ...
      setSources(
        [articles.sources, techArticles.sources, trendArticles.sources]
          .flat()
